<x-sidebar-layout :title="'Tambah CPMK'" :header="'Tambah CPMK — ' . $course->code">

    <div class="d-flex justify-content-between align-items-center mb-3">
        <p class="text-muted small mb-0">Untuk mata kuliah: <span class="fw-semibold">{{ $course->code }} — {{ $course->name }}</span></p>
        ...
        <form method="POST" action="{{ route('cpmks.store') }}">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="code" required value="{{ old('code') }}"
                           class="form-control @error('code') is-invalid @enderror" placeholder="CPMK 01">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" min="0" max="100" name="percentage" required value="{{ old('percentage') }}"
                           class="form-control @error('percentage') is-invalid @enderror" placeholder="20">
                ...
                <div class="col-12">
                    ...
                    <select name="lecturer_id" class="form-select">
                        ...
                        @foreach($lecturers as $l)
                            <option value="{{ $l->id }}" {{ old('lecturer_id') == $l->id ? 'selected' : '' }}>{{ $l->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mt-4 pt-3 border-top">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-3 gap-2">
                    <h3 class="h6 fw-bold mb-0">Indikator Kinerja <span class="text-danger">*</span></h3>
                    <div class="border rounded p-1 d-inline-flex" style="background:var(--obe-bg);">
                        <div class="form-check form-check-inline me-3 ms-2">
                            <input type="radio" name="indicator_weight_type" value="otomatis" id="iwt-auto"
                                   class="form-check-input" {{ old('indicator_weight_type', 'otomatis') === 'otomatis' ? 'checked' : '' }}
                                   onchange="toggleIndicatorWeightType()">
                            <label for="iwt-auto" class="form-check-label small">Otomatis</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input type="radio" name="indicator_weight_type" value="manual" id="iwt-manual"
                                   class="form-check-input" {{ old('indicator_weight_type') === 'manual' ? 'checked' : '' }}
                                   onchange="toggleIndicatorWeightType()">
                ...
                <div id="indicators-list">
                    @php
                        $oldDesc = old('indicator_descriptions', ['', '', '']);
                        $oldPct  = old('indicator_percentages', ['', '', '']);
                    @endphp
                    @foreach($oldDesc as $i => $desc)
                        <div class="row g-2 mb-2 indicator-row">
                            <div class="col-md-9">
                                <input type="text" name="indicator_descriptions[]" placeholder="Deskripsi Indikator {{ $i+1 }}"
                                       class="form-control indicator-desc-input" value="{{ $desc }}">
                            </div>
                            <div class="col-md-2">
                                <div class="input-group">
                                    <input type="number" step="0.01" min="0" max="100" name="indicator_percentages[]"
                                           placeholder="Bobot" class="form-control indicator-weight-input"
                                           value="{{ $oldPct[$i] ?? '' }}" oninput="calculateTotalIndicatorWeight()">
                                    <span class="input-group-text">%</span>
                                </div>
                            </div>
                            <div class="col-md-1 d-flex">
                                <button type="button" class="btn btn-sm btn-obe-outline w-100" onclick="removeIndicatorRow(this)" title="Hapus">
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <span class="badge bg-light text-dark border px-3 py-2" id="total-indicator-weight">Total: 100%</span>
                </div>
            </div>
...
function addIndicatorRow() {
    ...
    row.innerHTML = `
        <div class="col-md-9"><input type="text" name="indicator_descriptions[]" placeholder="Deskripsi Indikator ${n}" class="form-control indicator-desc-input"></div>
        ...
        <div class="col-md-1 d-flex"><button type="button" class="btn btn-sm btn-obe-outline w-100" onclick="removeIndicatorRow(this)" title="Hapus">×</button></div>`;
...
function removeIndicatorRow(btn) {
    if (document.querySelectorAll('.indicator-row').length <= 1) { alert('Minimal 1 indikator.'); return; }
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('form').addEventListener('submit', e => {
        ...
        if (t === 'manual') {
            let total = 0;
            document.querySelectorAll('.indicator-weight-input').forEach(i => total += parseFloat(i.value) || 0);
            if (Math.abs(total - 100) > 0.01) { e.preventDefault(); alert('Total bobot indikator (Manual) harus 100%. Saat ini: ' + total.toFixed(2) + '%'); }
        }
    });
